...
class Converter(object):
    ...
    def __format_punctuation_western(self, input):
        ...
        left_space = {'.':'.', ',':',', '!':'!', '?':'?', ';':';', ':':':', ')':')', ']':']', '」':'"', '”':'"', '--':'--'}
        right_space = {'(':'(', '[':'[', '「':'"', '“':'"'}
        input = ''.join(input.replace(punct_ch, punct_lat) for punct_ch, punct_lat in punctiation_mapping.items())
        input = ''.join([input.replace(' ' + left, space).replace(left, space) for left, space in left_space.items()])
        input = ''.join([input.replace(right + ' ', space).replace(right, space) for right, space in right_space.items()])
        return input
    ...
    def __format_punctuation_cjk(self, input):
        left_space = ['。', '．', '，', '、', '！', '？', '；', '：', '）', '］', '】', '」', '”', '--']
        right_space = ['（', '［', '【', '「', '“']
        input = ''.join([input.replace(' ' + punct + ' ', punct).replace(' ' + punct, punct) for punct in left_space])
        input = ''.join([input.replace(' ' + punct + ' ', punct).replace(punct + ' ', punct) for punct in right_space])
        return input
